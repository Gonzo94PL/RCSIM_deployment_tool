                                ...
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 text-xs">
                                  <div className="md:col-span-8 space-y-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                      <div>
                                        <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">
                                          {t('hid_controller')} ({subKey.endsWith('_gas') ? (t('lang_en') === 'English' ? 'Gas' : 'Gaz') : (t('lang_en') === 'English' ? 'Brake' : 'Hamulec')})
                                        </label>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                      ...
                                      <div>
                                        <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">
                                          {t('subtrim_label')} ({subKey.endsWith('_gas') ? (t('lang_en') === 'English' ? 'Gas' : 'Gaz') : (t('lang_en') === 'English' ? 'Brake' : 'Hamulec')}): {subKey.endsWith('_gas') ? item.subTrim : ((item as any).subTrimBrake || 0)}us
                                        </label>
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-xl space-y-6">
          ...
          <div className="p-4 bg-slate-950/40 border border-slate-800 rounded-xl space-y-5">
            ...
            <div className="border-t border-slate-800/80 pt-5 space-y-4">
              <h3 className="text-xs font-bold text-white uppercase tracking-wider">{t('lang_en') === 'English' ? 'Driving Assistant Buttons' : 'Przyciski Asystentów Jazdy'}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">
                    {t('btn_drive_mode')}
            ...
            {Object.keys(currentProfile.mappings).map((key) => {
              ...
              return (
                <div key={key} className="p-4 bg-slate-950/40 border border-slate-800 rounded-xl space-y-3.5">
                  <div className="flex justify-between items-center border-b border-slate-805/80 pb-2">
                    <span className="font-bold text-white text-xs">{key} (CH {item.outputChannel})</span>
                    <select
                    ...
                    >
                      <option value="bidirectional">{t('lang_en') === 'English' ? 'Bidirectional (Steering)' : 'Dwukierunkowa (np. Skręt)'}</option>
                      <option value="unidirectional">{t('lang_en') === 'English' ? 'Unidirectional (Throttle)' : 'Jednokierunkowa (np. Gaz)'}</option>
                      <option value="split_axis">{t('lang_en') === 'English' ? 'Split Axis (Gas/Brake)' : 'Oś Dzielona (np. Gaz/Ham.)'}</option>
                    </select>
                  </div>

                  <div className="grid grid-cols-2 gap-3 text-[11px] text-slate-400">
                    <div>
                      <span className="block mb-1">{t('lang_en') === 'English' ? 'Min Range:' : 'Zakres Min:'}</span>
                      <input
                        type="number"
                        min="1000"
                        max="1450"
                    ...
                    <div>
                      <span className="block mb-1">{t('lang_en') === 'English' ? 'Max Range:' : 'Zakres Max:'}</span>
                      <input
                        type="number"
                        min="1550"
                        max="2000"
        ...
        <div className="space-y-4">
          ...
          >
            <div>
              <span className="text-[9px] font-black text-indigo-400 uppercase bg-indigo-950 border border-indigo-900/50 px-2.5 py-1 rounded">
                {t('lang_en') === 'English' ? 'Active Profile' : 'Aktywny Profil'}
              </span>
              <h3 className="text-lg font-black text-white mt-2">{currentProfile.name}</h3>
              <p className="text-xs text-slate-400 mt-1">
                {t('lang_en') === 'English' ? 'Battery protection for package: ' : 'Zabezpieczenie baterii dla pakietu: '}{currentProfile.batteryType} LiPo
              </p>
            </div>
          ...
          <div className="space-y-2.5">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider pl-1">
              {t('lang_en') === 'English' ? 'List of profiles on disk' : 'Lista profili na dysku'}
            </p>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isCurrent && (
                        <span className="text-[8px] bg-indigo-650 text-white px-2 py-0.5 rounded font-bold uppercase tracking-wider">
                          {t('lang_en') === 'English' ? 'Active' : 'Aktywny'}
                        </span>
                      )}
                    </div>
                    <p className="text-[10px] text-slate-500 font-mono mt-1">
                      {t('lang_en') === 'English' ? 'Battery package: ' : 'Pakiet zasilania: '}{p.batteryType} LiPo • {t('lang_en') === 'English' ? 'Active mixes: ' : 'Aktywne miksy: '}{' '}
