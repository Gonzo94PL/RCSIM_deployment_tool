...
export default function RCTerminalPage() {
  ...
  const [profiles, setProfiles] = useState<Profile[]>([
    {
      ...
      cruiseBtn: '',
      mappings: {
        'Throttle': { name: 'Throttle', device: 'usb-1', axis: 'Oś Z', outputChannel: 3, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 10, type: 'unidirectional' },
    ...
    },
    {
      ...
      cruiseBtn: '',
      mappings: {
        'Throttle': { name: 'Throttle', device: 'usb-1', axis: 'Oś Z', outputChannel: 3, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 5, type: 'unidirectional' },
    ...
    {
      id: 'car-crawler-trx4',
      name: 'Car_Crawler_TRX4',
      batteryType: '3S',
      gearboxConfig: {
        mode: 'sequential',
        num_forward_gears: 2,
        reverse_throttle_limit: 0.4,
        btn_up: 'BTN_TR',
        btn_down: 'BTN_TL',
      },
      driveModeBtn: 'BTN_WEST',
      cruiseBtn: 'BTN_NORTH',
      mappings: {
        'Throttle': { name: 'Throttle', device: 'usb-1', axis: 'Oś Y', outputChannel: 2, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 15, type: 'unidirectional' },
        'Aileron': { name: 'Aileron', device: 'usb-1', axis: 'Oś X', outputChannel: 1, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 25 },
        'Elevator': { name: 'Elevator', device: 'None', axis: 'Brak', outputChannel: 3, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 0 },
        'Rudder': { name: 'Rudder', device: 'None', axis: 'Brak', outputChannel: 4, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 0 },
        'Aux1': { name: 'Aux1', device: 'usb-1', axis: 'BTN_A', outputChannel: 5, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 0 },
        'Aux2': { name: 'Aux2', device: 'usb-1', axis: 'BTN_B', outputChannel: 6, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 0 },
        'Aux3': { name: 'Aux3', device: 'None', axis: 'Brak', outputChannel: 7, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 0 },
        'Aux4': { name: 'Aux4', device: 'None', axis: 'Brak', outputChannel: 8, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 0 },
      }
    }
      ...
      if (res.ok) {
        ...
        if (config.profiles) {
          const loadedProfiles: Profile[] = Object.keys(config.profiles).map(key => {
            ...
            const ffbAutocenter: { [deviceName: string]: number } = {};
            const steeringRange: { [deviceName: string]: number } = {};
            const hardwareRange: { [deviceName: string]: number } = {};
            if (bp.inputs) {
              bp.inputs.forEach((input: any) => {
                const devName = input.device_name;
                if (input.ffb_autocenter !== undefined) {
                  ffbAutocenter[devName] = input.ffb_autocenter;
                }
                if (input.steering_range !== undefined) {
                  steeringRange[devName] = input.steering_range;
                }
                if (input.hardware_range !== undefined) {
                  hardwareRange[devName] = input.hardware_range;
                }
                if (input.mappings) {
                  input.mappings.forEach((m: any) => {
                    ...
                    if (mappings[name]) {
                      mappings[name] = {
                        ...
                        type: m.type || (name.toLowerCase() === 'throttle' ? 'unidirectional' : 'bidirectional'),
                        deviceBrake: m.deviceBrake || 'None',
                        axisBrake: m.axisBrake || 'Brak',
                        reverseBrake: m.reverseBrake || false,
                        subTrimBrake: m.subTrimBrake || 0,
                        expoBrake: m.expoBrake || 0,
                        failsafeMode: m.failsafe_mode || 'center',
                        failsafeValue: m.failsafe_value !== undefined ? m.failsafe_value : 1500,
                        rawMin: m.raw_min !== undefined ? m.raw_min : null,
                        rawMax: m.raw_max !== undefined ? m.raw_max : null,
                        rawCenter: m.raw_center !== undefined ? m.raw_center : null,
                        rawMinBrake: m.raw_min_brake !== undefined ? m.raw_min_brake : null,
                        rawMaxBrake: m.raw_max_brake !== undefined ? m.raw_max_brake : null
                      };
                    }
                  });
                }
            ...
            return {
              ...
              armBtn: bp.arm_btn || '',
              ffbAutocenter: ffbAutocenter,
              steeringRange: steeringRange,
              hardwareRange: hardwareRange,
            };
          });
  ...
  const saveConfigToBackend = async (
    ...
    updatedProfiles.forEach(p => {
      ...
      Object.keys(p.mappings).forEach(key => {
        ...
        if (m.device && m.device !== 'None' && m.device !== 'Brak') {
          ...
          inputsMap[m.device].push({
            ...
            subTrimBrake: m.subTrimBrake || 0,
            expoBrake: m.expoBrake || 0,
            failsafe_mode: m.failsafeMode || 'center',
            failsafe_value: m.failsafeValue !== undefined ? m.failsafeValue : 1500,
            raw_min: m.rawMin !== undefined ? m.rawMin : null,
            raw_max: m.rawMax !== undefined ? m.rawMax : null,
            raw_center: m.rawCenter !== undefined ? m.rawCenter : null,
            raw_min_brake: m.rawMinBrake !== undefined ? m.rawMinBrake : null,
            raw_max_brake: m.rawMaxBrake !== undefined ? m.rawMaxBrake : null
          });
        }
      });
      ...
      const inputsList = Object.keys(inputsMap).map(devName => ({
        device_name: devName,
        ffb_autocenter: p.ffbAutocenter?.[devName] !== undefined ? p.ffbAutocenter[devName] : 0,
        steering_range: p.steeringRange?.[devName] !== undefined ? p.steeringRange[devName] : 900,
        hardware_range: p.hardwareRange?.[devName] !== undefined ? p.hardwareRange[devName] : 900,
  ...
  const handleCreateProfile = async (e: React.FormEvent) => {
    ...
    const defaultMappings: { [key: string]: ChannelMapping } = {
      'Throttle': { name: 'Throttle', device: 'None', axis: 'Brak', outputChannel: 3, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 10, type: 'unidirectional' },
      'Aileron': { name: 'Aileron', device: 'None', axis: 'Brak', outputChannel: 1, reverse: false, subTrim: 0, epaMin: 1100, epaMax: 1900, expo: 20 },
      'Elevator': { name: 'Elevator', device: 'None', axis: 'Brak', outputChannel: 2, reverse: false, subTrim: 0, epaMin: 1100, epaMax: 1900, expo: 20 },
      'Rudder': { name: 'Rudder', device: 'None', axis: 'Brak', outputChannel: 4, reverse: false, subTrim: 0, epaMin: 1100, epaMax: 1900, expo: 10 },
      'Aux1': { name: 'Aux1', device: 'None', axis: 'Brak', outputChannel: 5, reverse: false, subTrim: 0, epaMin: 1000, epaMax: 2000, expo: 0 },
      ...
      <div className="max-w-7xl mx-auto px-6 py-8" id="rc-main-workspace grid">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <nav className="col-span-12 lg:col-span-3 space-y-3" id="rc-tab-navigation">
            <p className="text-[10px] font-black text-slate-500 tracking-wider uppercase pl-2">{lang === 'en' ? 'Configuration Modules' : 'Moduły Konfiguracji'}</p>
