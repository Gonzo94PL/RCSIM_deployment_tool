...
class SystemdWatchdogThread(threading.Thread):
    """Wątek tła wysyłający sygnały watchdog do systemd."""
    def __init__(self, interval: float = 2.0):
        super().__init__(daemon=True, name="SystemdWatchdog")
...
def create_app(
    ...
    async def _lifespan(app: FastAPI):
        ...
        if os.environ.get("NOTIFY_SOCKET"):
            watchdog = SystemdWatchdogThread(interval=2.0)
            watchdog.start()
            _notify_systemd("READY=1\nSTATUS=Web API server running and monitored by watchdog")
        yield
        # Shutdown cleanup
        if watchdog:
            watchdog.stop()
    ...
    def get_status() -> dict[str, Any]:
        ...
        rssi, lq = rc_state.get_telemetry()
